...
func Encrypt(key *rsa.PublicKey, secretMessage string) (string, error) {
	...
	ciphertext, err := rsa.EncryptOAEP(sha512.New(), rand.Reader, key, []byte(secretMessage), label)
	if err != nil {
		return "", err
	}
...
func Decrypt(privateKey *rsa.PrivateKey, cipherText string) (string, error) {
	...
	plaintext, err := rsa.DecryptOAEP(sha512.New(), rand.Reader, privateKey, ct, label)
	if err != nil {
		return "", err
	}
