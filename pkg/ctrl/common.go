...
var skExpire = time.Minute * time.Duration(conf.Conf.RefreshTokenAge)
...
func dbUserByAddr(r *Ctrl, c *gin.Context, addr string) (user model.User, ok bool) {
	ok = false
	err := r.DB.Model(&user).First(&user, "addr=?", addr).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			web.BadRes(c, util.ErrNoRec)
		} else {
