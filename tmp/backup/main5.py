...
def getprojectinfo(project, url, currencys):
    ...
    if e["code"] != 200:
        print("project服务异常")
        raise( ValueError("project服务异常: %s"%e["code"]))
...
def matchParams(daily_dict, tvls_dict, aprs_dict, currencys_dict):
    ...
    argsa = []
    for a in aprs_name:
        argsa.append(float(aprs_dict.get(a, 0)))
    print(aprs_name)
    print(argsa)
    for k, v in dict(zip(aprs_name, argsa)).items():
        print("%s:%s" % (k, v))
    print("---------------------ENDS--------------------")
    return argsp, argsr, argstt, argsa
...
def outputReTask():
    ...
    argsp, argsr, argstt, argsa = matchParams(daily_dict, tvls_dict, aprs_dict, currencys_dict)
    ...
    "busd_amt_pancake_bnb",
    "busd_amt_biswap_bnb",
    "busd_amt_pancake_cake",
    "bnb_amt_biswap_usdt",
    "bnb_amt_pancake_usdt",
    "btcb_amt_biswap_usdt",
    "eth_amt_biswap_usdt",
    "usdt_amt_biswap_eth",
    "usdt_amt_biswap_btcb",
    "usdt_amt_biswap_bnb",
    "usdt_amt_pancake_bnb",
    "usdt_amt_pancake_cake",
    "cake_amt_pancake_usdt",
    "cake_amt_pancake_busd")

    for k,v in dict(zip(Y, X)).items():
        print("%s\t:\t%s"%(k,v))

    bnb, cake, btcb, eth, busd, usdt = argsp
    print("price btc:%f eth:%f usdt:%f"%(btcb, eth, usdt))
    print("biswap btc:%f  usdt:%f"%(X[7], X[10]))
    print("biswap eth:%f  usdt:%f"%(X[8], X[9]))
